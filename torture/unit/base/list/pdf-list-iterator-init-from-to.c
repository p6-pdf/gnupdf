...
/*
 * Test: pdf_list_iterator_init_from_to_003
 * Description:
 *   Try to get an iterator given a NULL iterator pointer.
 * Success condition:
 *   Returns PDF_EBADDATA
 */
START_TEST (pdf_list_iterator_init_from_to_003)
{
  pdf_list_t *list;
  int elem, elem2;
  pdf_error_t *error = NULL;
  ...
  fail_if (pdf_list_iterator_init_from_to (NULL, list, 0, 2, &error) == PDF_TRUE);
  fail_if (error == NULL);
...
/*
 * Test: pdf_list_iterator_init_from_to_004
 * Description:
 *   Try to get an iterator given a NULL list pointer.
 * Success condition:
 *   Returns PDF_EBADDATA
 */
START_TEST (pdf_list_iterator_init_from_to_004)
{
  pdf_list_iterator_t itr;
  pdf_error_t *error = NULL;
  ...
  fail_if (pdf_list_iterator_init_from_to (&itr, NULL, 0, 2, &error) == PDF_TRUE);
  fail_if (error == NULL);
  fail_if (pdf_error_get_status (error) != PDF_EBADDATA);
}
END_TEST
...
test_pdf_list_iterator_init_from_to (void)
{
  ...
  tcase_add_test (tc, pdf_list_iterator_init_from_to_003);
